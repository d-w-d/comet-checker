...
export class HeaderComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  hoverMenuSubscription: Subscription;

  @Input()
  isHoverMenuEnabled: boolean;
  // hoverMenu$: Observable<boolean>;

  @ViewChildren('mainbutton')
  mainbuttons: QueryList<ElementRef>;
  _mainbuttons: ElementRef[];
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      this._mainbuttons = this.mainbuttons.toArray();
    });
  }
   ...
   */
  handleInteraction(trigger, action: NavActions, event?: Event, childNumber?: number) {
