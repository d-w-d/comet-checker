...
import { createComponent, RenderResult, fireEvent } from '@angular-extensions/testing-library';

import { MockStore, TestingModule } from '@testing/utils';
import { NotificationService } from '@app/core';

import { ActionTodosFilter, ActionTodosRemoveDone, ActionTodosToggle } from '../todos.actions';
import { ITodosState } from '../todos.model';
...
describe('TodosComponent', () => {
  ...
  it('should be created with 0 todos', () => {
    ...
    expect((component.queryAllByTestId as any)('todo-item').length).toBe(0);
  });
  ...
  it('should display todos', () => {
    ...
    expect((component.queryAllByTestId as any)('todo-item').length).toBe(1);
  ...
  it('should dispatch filter todo action', () => {
    ...
    fireEvent.click(component.fixture.debugElement.query(By.css('[aria-label="show active items"]')).nativeElement);
  ...
  it('should disable remove done todos button if no todo is done', () => {
    ...
    expect((component.getByLabelText('remove done items') as HTMLInputElement).disabled).toBeFalsy();
  });
  ...
  it('should disable add new todo button if input length is less than 4', () => {
    ...
    expect((component.getByLabelText('add todo') as HTMLInputElement).disabled).toBeTruthy();
  });
...
function createState(todoState: ITodosState) {
  return {
    examples: {
      todos: todoState
    }
